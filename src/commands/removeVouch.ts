/**
 * /remove-vouch command - Remove the most recent vouch report (permission-gated)
 */
...
export const data = new SlashCommandBuilder()
  .setName('remove-vouch')
  .setDescription('Remove the most recent vouch report for a user (requires Manage Roles)')
  .addUserOption((option) =>
    option.setName('user').setDescription('The user to remove a vouch report from').setRequired(true)
  );
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    // Get latest report
    const reportResult = await db.getLatestReport(interaction.guild.id, targetUser.id, 'vouch');
    if (!reportResult.success || !reportResult.data) {
      await interaction.reply({
        embeds: [createErrorEmbed('No vouch reports found for this user.')],
        ephemeral: true,
      });
      return;
    }
    ...
    if (!removeResult.success) {
      await interaction.reply({
        embeds: [createErrorEmbed(removeResult.error || 'Failed to remove vouch report.')],
        ephemeral: true,
      });
      return;
    }
    ...
    logError('remove-vouch command', error);
    await interaction.reply({
